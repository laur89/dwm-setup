...
class cpu_and_mem(Segment):
    ...
    def __init__(self, color):
        ...
        self.build_module(output + '\\u00a0' + self.icons.get('miniarrow')  + '\\u00a0' + self.icons.get('ram') + '\\u00a0' + mem, color)
...
class wx(Segment):
    ...
    def __init__(self, color):
        ...
        try:
            ...
            temp=wx_info[index+7:index2] + '\\u00B0C'   # Degree symbol
            #print(temp)
...
nw = 'nw ' + str(network('black_yellow'))+'\n'
...
while 1:
    if mode == '1':
        #cpu = 'cpu '+ str(CPU('black_yellow', False))+'\n' # if second parameter is True, then CPU() returns recource usage for all cores
        bat = 'bat ' + str(bat_('black_gray'))+'\n'
        #mem = 'mem '+ str(Mem('black_blue'))+'\n'
        time_ = 'time ' + str(Time('white'))+'\n'
        
        date = 'date ' + str(Date('black_gray'))+'\n'
        vol = 'vol ' + str(Vol('black_blue'))+'\n'
        music = 'music '+str(Music('black_yellow'))+'\n'
        weather = 'wx '+str(wx('black_gray'))+'\n'
        c_a_m = 'c_a_m ' + str(cpu_and_mem('black_yellow'))+'\n'
        
        # Write also empty shells:
        #nw = 'nw \n'

        f = open(barfile, 'w', encoding='utf-8')

        for i in [c_a_m, bat, weather, music, vol, time_, date, nw]:
            f.write(i)
        f.close()
        del bat
        ...
        for i in [ nw, weather, vol, time_, date, bat, music, c_a_m ]:
            f.write(i)
        f.close()
